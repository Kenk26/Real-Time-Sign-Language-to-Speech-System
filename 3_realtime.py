"""
Pipeline:
  Camera → MediaPipe Hands/Face (Tasks API) → Landmarks → LSTM → Tokens
...
def extract_landmarks(result) -> np.ndarray:
    ...
    Missing hands are padded with zeros. Includes safeguard for MediaPipe glitches.
    """
...
def run(model_path: str = "models/sign_model.keras",
    ...
    if not os.path.exists(model_path):
        ...
        print("        Run: training_notebook.ipynb first.")
        return
    ...
    while True:
        ...
        # ── Hand landmark extraction (Tasks API) ──────────────────
        ...
        if hand_detected:
            ...
            for hand_lms in results.hand_landmarks:
                draw_skeleton(frame, hand_lms, color=PALETTE["accent"])
        ...
        if (len(frame_buffer) == SEQUENCE_LENGTH and hand_detected):
            seq = np.array(list(frame_buffer), dtype=np.float32)  # (30, 126)
            seq = normalize(seq)
            inp = np.expand_dims(seq, 0)          # (1, 30, 126)
